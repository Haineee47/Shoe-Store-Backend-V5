...
class DomainRepositoryDependencyBoundaryTest {
  ...
  void domainRepositoryShouldRemainAnInterface() {
    assertThat(TestAggregateRepository.class.isInterface())
        .isTrue();
  }
  ...
  private void assertAllowedAnnotation(
    ...
    assertThat(isForbiddenClassName(annotationName))
        .as(
            "%s must not declare framework annotation %s",
            owner,
            annotationName)
        .isFalse();
  }

  private void assertAllowedDependency(
      Type dependency,
      String location) {

    assertThat(containsForbiddenDependency(dependency))
        .as(
            "%s must not reference forbidden dependency %s",
            location,
  ...
  private boolean containsForbiddenDependency(Type type) {
    if (type instanceof Class<?> typeClass) {
      if (typeClass.isArray()) {
        return containsForbiddenDependency(
            typeClass.getComponentType());
      }

      return isForbiddenClassName(
          typeClass.getName());
    }

    if (type instanceof ParameterizedType parameterizedType) {
      if (containsForbiddenDependency(
      ...
      return Arrays.stream(
          ...
          .anyMatch(
              this::containsForbiddenDependency);
    }
  ...
  private boolean containsForbiddenSimpleType(Type type) {
    if (type instanceof Class<?> typeClass) {
      if (typeClass.isArray()) {
        return containsForbiddenSimpleType(
            typeClass.getComponentType());
      }

      return FORBIDDEN_SIMPLE_TYPE_NAMES.contains(
          typeClass.getSimpleName());
    }

    if (type instanceof ParameterizedType parameterizedType) {
      if (containsForbiddenSimpleType(
          parameterizedType.getRawType())) {
      ...
      return Arrays.stream(
              parameterizedType.getActualTypeArguments())
          .anyMatch(
              this::containsForbiddenSimpleType);
    }
  ...
  private boolean isForbiddenClassName(
      String className) {

    return FORBIDDEN_PACKAGE_PREFIXES.stream()
            .anyMatch(className::startsWith)
        || FORBIDDEN_PACKAGE_SEGMENTS.stream()
