...
class DomainServiceBusinessLogicTest {
    ...
    void shouldFullyAllocateWhenAvailableQuantityExceedsRequestedQuantity() {
        ...
        assertThat(decision.status())
                .isEqualTo(TestAllocationStatus.FULLY_ALLOCATED);

        assertThat(decision.isFullyAllocated())
                .isTrue();
    }
    ...
    void shouldPartiallyAllocateWhenAvailableQuantityIsLessThanRequestedQuantity() {
        ...
        assertThat(decision.unfulfilledQuantity())
                .isEqualTo(new TestAllocationQuantity(6));

        assertThat(decision.status())
                .isEqualTo(TestAllocationStatus.PARTIALLY_ALLOCATED);
    ...
    void shouldPreserveRequestedQuantityConservation() {
        ...
        int reconstructedRequested =
                Math.addExact(
                        decision.allocatedQuantity().value(),
                        decision.unfulfilledQuantity().value()
                );

        assertThat(reconstructedRequested)
                .isEqualTo(requested.value());
    }
    ...
    void shouldPreserveAvailableQuantityConservation() {
        ...
        int reconstructedAvailable =
                Math.addExact(
                        decision.allocatedQuantity().value(),
                        decision.remainingAvailableQuantity().value()
                );

        assertThat(reconstructedAvailable)
                .isEqualTo(available.value());
    }

    @Test
    void shouldNeverAllocateMoreThanRequestedQuantity() {
        TestAllocationQuantity requested =
                new TestAllocationQuantity(7);

        TestAllocationQuantity available =
                new TestAllocationQuantity(20);

        TestAllocationDecision decision =
                allocationPolicy.allocate(requested, available);

        assertThat(decision.allocatedQuantity().value())
                .isLessThanOrEqualTo(requested.value());
    }
    ...
    void shouldNeverAllocateMoreThanAvailableQuantity() {
        ...
        assertThat(decision.allocatedQuantity().value())
                .isLessThanOrEqualTo(available.value());
    }
}
