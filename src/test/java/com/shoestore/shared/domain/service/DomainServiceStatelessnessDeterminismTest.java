...
class DomainServiceStatelessnessDeterminismTest {
    ...
    void shouldProduceEquivalentDecisionAcrossDifferentServiceInstances() {
        TestAllocationPolicy firstPolicy =
                new TestAllocationPolicy();

        TestAllocationPolicy secondPolicy =
                new TestAllocationPolicy();

        TestAllocationDecision firstDecision =
                firstPolicy.allocate(
                        new TestAllocationQuantity(9),
                        new TestAllocationQuantity(4)
                );

        TestAllocationDecision secondDecision =
                secondPolicy.allocate(
                        new TestAllocationQuantity(9),
                        new TestAllocationQuantity(4)
                );
    ...
    void shouldNotDependOnPreviousInvocation() {
        ...
        assertThat(secondTargetDecision)
                .isEqualTo(firstTargetDecision);
    }
    ...
    void shouldNotDependOnInvocationOrder() {
        ...
        assertThat(firstCaseFromFirstOrder)
                .isEqualTo(firstCaseFromReverseOrder);

        assertThat(secondCaseFromFirstOrder)
                .isEqualTo(secondCaseFromReverseOrder);
    }
