...
/**
 * Stateless Domain Service fixture that determines how much of a requested
 * quantity can be allocated from an available quantity.
 */
public final class TestAllocationPolicy {
    ...
    public TestAllocationDecision allocate(
        ...
        Objects.requireNonNull(
                requestedQuantity,
                "requestedQuantity must not be null"
        );
        ...
        int allocatedValue = Math.min(
                requestedQuantity.value(),
                availableQuantity.value()
        );

        TestAllocationQuantity allocatedQuantity =
                new TestAllocationQuantity(allocatedValue);
        ...
        TestAllocationQuantity unfulfilledQuantity =
                requestedQuantity.subtract(allocatedQuantity);

        TestAllocationStatus status = determineStatus(
                allocatedQuantity,
                unfulfilledQuantity
        );
