...
public record TestAllocationDecision(
  ...
  public TestAllocationDecision {
    Objects.requireNonNull(allocatedQuantity, "allocatedQuantity must not be null");

    Objects.requireNonNull(
        remainingAvailableQuantity, "remainingAvailableQuantity must not be null");

    Objects.requireNonNull(unfulfilledQuantity, "unfulfilledQuantity must not be null");

    Objects.requireNonNull(status, "status must not be null");

    validateStatusConsistency(allocatedQuantity, unfulfilledQuantity, status);
  }
  ...
  private static void validateStatusConsistency(
    ...
    switch (status) {
      ...
      case PARTIALLY_ALLOCATED -> {
        if (allocatedQuantity.isZero() || unfulfilledQuantity.isZero()) {
          throw new IllegalArgumentException(
