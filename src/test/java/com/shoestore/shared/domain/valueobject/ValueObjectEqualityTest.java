...
class ValueObjectEqualityTest {
  ...
  void shouldNotBeEqualToDifferentType() {
    ...
    assertThat(value.equals("Nike")).isFalse();
  }
  ...
  void shouldReplaceExistingValueInHashMap() {
    ...
    map.put(new TestScalarValue("Nike"), 1);

    map.put(new TestScalarValue("Nike"), 2);

    assertThat(map).hasSize(1);

    assertThat(map.get(new TestScalarValue("Nike"))).isEqualTo(2);
  }
  ...
  void quantityShouldUseValueEquality() {
    ...
    var first = new TestQuantity(5);

    var second = new TestQuantity(5);
