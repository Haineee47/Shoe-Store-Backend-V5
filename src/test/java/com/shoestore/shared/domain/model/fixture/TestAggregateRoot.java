...
/** Aggregate test fixture used to verify aggregate-root and domain-entity conventions. */
public final class TestAggregateRoot extends BaseEntity implements AggregateRoot {
  ...
  private int requireValidPosition(int position) {
    if (position < 0 || position >= children.size()) {
      throw new IllegalArgumentException("child position is outside aggregate boundaries");
    }
  ...
  private void ensureActive() {
    if (status != TestAggregateStatus.ACTIVE) {
      throw new IllegalStateException("only active aggregate may modify children");
    }
  }

  private void ensureNotArchived() {
    if (status == TestAggregateStatus.ARCHIVED) {
      throw new IllegalStateException("archived aggregate cannot change lifecycle state");
    }
  }

  private static String requireName(String value) {
    Objects.requireNonNull(value, "aggregate name must not be null");
