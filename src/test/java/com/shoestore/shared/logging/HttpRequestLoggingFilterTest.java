...
class HttpRequestLoggingFilterTest {
  ...
  void shouldLogSuccessfulRequestAtInfoLevel() throws Exception {
    ...
    verify(logger, never()).error(org.mockito.ArgumentMatchers.anyString());

    assertThat(messageCaptor.getValue())
        .startsWith(
            "HTTP request completed method=GET " + "path=/api/products status=200 durationMs=");
  }
  ...
  void shouldLogClientErrorAtInfoLevel() throws Exception {
    ...
    assertThat(messageCaptor.getValue())
        .startsWith(
            "HTTP request completed method=POST " + "path=/api/orders status=400 durationMs=");
  }
  ...
  void shouldLogServerErrorAtErrorLevel() throws Exception {
    ...
    assertThat(messageCaptor.getValue())
        .startsWith(
            "HTTP request failed method=GET " + "path=/api/products status=500 durationMs=");
  }
  ...
  void shouldLogEvenWhenFilterChainThrowsException() throws Exception {
    ...
    org.mockito.Mockito.doThrow(new RuntimeException("Unexpected failure"))
        .when(filterChain)
  ...
  void shouldRemoveContextPathFromLoggedPath() throws Exception {
    MockHttpServletRequest request = new MockHttpServletRequest("GET", "/shoe-store/api/products");
