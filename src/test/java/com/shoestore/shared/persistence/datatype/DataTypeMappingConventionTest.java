...
class DataTypeMappingConventionTest {
  ...
  void shouldDeclareExplicitStringLength() throws Exception {
    ...
    assertThat(column.length()).isEqualTo(150);
  }
  ...
  @Test
  void shouldMapMoneyWithExplicitPrecisionAndScale() throws Exception {
  ...
  @Test
  void shouldMapMeasurementWithExplicitPrecisionAndScale() throws Exception {
  ...
  @Test
  void shouldUseCorrectTemporalJavaTypes() throws Exception {
    assertThat(getField("occurredAt").getType()).isEqualTo(Instant.class);

    assertThat(getField("businessDate").getType()).isEqualTo(LocalDate.class);

    assertThat(getField("openingTime").getType()).isEqualTo(LocalTime.class);
  }
  ...
  void shouldStoreEnumAsString() throws Exception {
    ...
    assertThat(getColumn("status").length()).isEqualTo(50);
  }

  private Field getField(String name) throws NoSuchFieldException {
    return DataTypeTestEntity.class.getDeclaredField(name);
  }

  private Column getColumn(String fieldName) throws NoSuchFieldException {
    return getField(fieldName).getAnnotation(Column.class);
  }
}
