...
@Table(
    ...
    uniqueConstraints = {
      @UniqueConstraint(
          name = "uk_relationship_children_parent_position",
          columnNames = {"parent_id", "position"})
    },
    indexes = {@Index(name = "idx_relationship_children_parent_id", columnList = "parent_id")})
public class RelationshipChildTestEntity extends AuditableEntity {
  ...
  @JoinColumn(
      ...
      nullable = false,
      updatable = false,
      foreignKey = @ForeignKey(name = "fk_relationship_children_parent"))
  private RelationshipParentTestEntity parent;

  protected RelationshipChildTestEntity() {}

  public RelationshipChildTestEntity(String name, int position) {
  ...
  void attachTo(RelationshipParentTestEntity parent) {
    ...
    if (this.parent != null && this.parent != parent) {
      throw new IllegalStateException("Child is already attached to another parent");
    }
