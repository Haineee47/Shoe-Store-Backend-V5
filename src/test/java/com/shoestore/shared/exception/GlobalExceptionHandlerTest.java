...
class GlobalExceptionHandlerTest {
  ...
  @Test
  void shouldReturnValidationErrorResponseAndLogWarning() throws Exception {
  ...
  @Test
  void shouldReturnMalformedRequestResponseAndLogWarning() throws Exception {
  ...
  @Test
  void shouldReturnMethodNotAllowedResponseAndLogWarning() throws Exception {

    mockMvc
        .perform(delete("/test/validation"))
        .andExpect(status().isMethodNotAllowed())
        .andExpect(jsonPath("$.success").value(false))
        .andExpect(jsonPath("$.status").value(405))
        .andExpect(jsonPath("$.error").value("Method Not Allowed"))
        .andExpect(
            jsonPath("$.message").value("HTTP method is not supported " + "for this endpoint"))
  ...
  @Test
  void shouldReturnApplicationErrorResponseAndLogWarning() throws Exception {

    mockMvc
        .perform(get("/test/application-exception"))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.success").value(false))
        .andExpect(jsonPath("$.status").value(400))
        .andExpect(jsonPath("$.error").value("Bad Request"))
        .andExpect(jsonPath("$.message").value("Test application failure"))
        .andExpect(jsonPath("$.errorCode").value("TEST_APPLICATION_ERROR"))
        .andExpect(jsonPath("$.path").value("/test/application-exception"))
        .andExpect(jsonPath("$.timestamp").exists());
  ...
  @Test
  void shouldReturnServerApplicationErrorAndLogStackTrace() throws Exception {

    mockMvc
        .perform(get("/test/server-application-exception"))
        .andExpect(status().isInternalServerError())
        .andExpect(jsonPath("$.success").value(false))
        .andExpect(jsonPath("$.status").value(500))
        .andExpect(jsonPath("$.error").value("Internal Server Error"))
        .andExpect(jsonPath("$.message").value("Test server application failure"))
        .andExpect(jsonPath("$.errorCode").value("TEST_SERVER_ERROR"))
        .andExpect(jsonPath("$.path").value("/test/server-application-exception"))
        .andExpect(jsonPath("$.timestamp").exists());
  ...
  @Test
  void shouldReturnInternalServerErrorWithoutExposingExceptionMessage() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath("$.message").value(org.hamcrest.Matchers.not("Sensitive database detail")))
  ...
  static class TestController {
    ...
    void throwApplicationException() {
      throw new ApplicationException(TestErrorCode.APPLICATION_ERROR);
    }

    @GetMapping("/server-application-exception")
    void throwServerApplicationException() {
      throw new ApplicationException(TestErrorCode.SERVER_ERROR);
    }
  ...
  enum TestErrorCode implements ErrorCode {
    APPLICATION_ERROR("TEST_APPLICATION_ERROR", "Test application failure", HttpStatus.BAD_REQUEST),

    SERVER_ERROR(
        "TEST_SERVER_ERROR", "Test server application failure", HttpStatus.INTERNAL_SERVER_ERROR);
