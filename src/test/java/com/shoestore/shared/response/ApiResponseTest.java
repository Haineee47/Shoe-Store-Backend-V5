...
class ApiResponseTest {
  ...
  void shouldCreateSuccessfulResponseWithData() {
    Map<String, String> data = Map.of("id", "product-001");
  ...
  void shouldCreateSuccessfulResponseWithCustomMessageAndData() {
    ...
    ApiResponse<String> response = ApiResponse.success("Resource created successfully", data);
  ...
  void shouldCreateSuccessfulResponseWithoutData() {
    ApiResponse<Void> response = ApiResponse.success("Resource deleted successfully");

    assertThat(response.success()).isTrue();
    assertThat(response.message()).isEqualTo("Resource deleted successfully");
