...
class DomainServiceArchitectureTest {
    ...
    private static final String MODULE_DOMAIN_SERVICES =
            "..modules..domain.service..";

    private static final String SHARED_DOMAIN_SERVICES =
            "..shared.domain.service..";

    private static final String[] DOMAIN_SERVICE_PACKAGES = {
            MODULE_DOMAIN_SERVICES,
    ...
    void domainServicesShouldUseDomainOrientedNames() {
        ArchRule rule =
                classes()
                        ...
                        .because(
                                "Domain Services must use names that express business responsibility"
                        );
    ...
    void domainServicesShouldNotDependOnSpring() {
        noClasses()
                ...
                .allowEmptyShould(true)
                .because(
                        "pure Domain Services must remain independent of Spring"
                )
    ...
    void domainServicesShouldNotDependOnPersistence() {
        noClasses()
                ...
                .allowEmptyShould(true)
                .because(
                        "persistence access belongs to the application and infrastructure layers"
                )
                .check(productionClasses);
    }
    ...
    void domainServicesShouldNotDependOnOuterLayers() {
        noClasses()
                ...
                .allowEmptyShould(true)
                .because(
                        "outer layers must depend on domain, never the reverse"
                )
                .check(productionClasses);
    }
    ...
    void domainServicesShouldNotDependOnTransportOrIntegrationApis() {
        noClasses()
                ...
                .allowEmptyShould(true)
                .because(
                        "transport, messaging and caching are not domain concerns"
                )
                .check(productionClasses);
    }
    ...
    void domainServicesShouldNotDependOnSecurityOrLogging() {
        noClasses()
                ...
                .allowEmptyShould(true)
                .because(
                        "security context and logging belong at application boundaries"
                )
                .check(productionClasses);
    }
    ...
    void domainServicesShouldNotDeclareMutableStaticState() {
        fields()
                ...
                .areStatic()
                .should(
                        beImmutableStaticConstant()
                )
                .allowEmptyShould(true)
                .because(
                        "Domain Services must not share mutable runtime state"
                )
                .check(productionClasses);
    }

    private static ArchCondition<JavaField>
    beImmutableStaticConstant() {
        return new ArchCondition<>(
                "be a static final immutable constant"
        ) {
            @Override
            public void check(
                    JavaField field,
                ...
                boolean immutableType =
                        field.getRawType().isPrimitive()
                                ...
                                || IMMUTABLE_CONSTANT_TYPES.contains(
                                        field.getRawType().getName()
                                );

                boolean valid =
                        finalField && immutableType;

                String message =
                        field.getFullName()
                                + " must be final and use a supported immutable constant type";
