...
class DomainExceptionArchitectureTest {
    ...
    void sharedDomainExceptionFoundationShouldRemainIndependent() {
        ArchRule rule = noClasses()
                ...
                .because(
                        "shared domain exception foundations must remain "
    ...
    void domainErrorCodeImplementationsShouldResideInDomainExceptionPackages() {
        ArchRule rule = classes()
                ...
                .because(
                        "domain error-code ownership belongs to the "
    ...
    void domainErrorCodeImplementationsShouldBeEnums() {
        ArchRule rule = classes()
                ...
                .beEnums()
                .because(
                        "module domain errors use closed and explicit "
                                + "enum identities"
                )
                .allowEmptyShould(true);
    ...
    void controllersShouldNotDependOnDomainExceptions() {
        ArchRule rule = noClasses()
                ...
                .because(
                        "controllers must consume application-level "
                                + "failure contracts"
                )
                .allowEmptyShould(true);
    ...
    void domainExceptionTranslatorsShouldResideInApplicationLayer() {
        ArchRule rule = classes()
                ...
                .because(
                        "domain-to-application translation belongs to "
                                + "the module application boundary"
                )
                .allowEmptyShould(true);
    ...
    void domainExceptionTranslatorsShouldRemainInfrastructureFree() {
        ArchRule rule = noClasses()
                ...
                .because(
                        "domain exception translators perform explicit "
    ...
    constructDomainExceptions() {
        return new ArchCondition<>(
                "construct DomainException implementations"
        ) {
            @Override
            public void check(
                    JavaClass javaClass,
                ...
                for (JavaConstructorCall call
                        : javaClass.getConstructorCallsFromSelf()) {

                    if (!call.getTargetOwner()
                            .isAssignableTo(DomainException.class)) {
                        continue;
                    }
